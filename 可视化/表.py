...
def render_content(tab):
    ...
    if tab == 'tab-overview':
        return html.Div([
            # 第一行图表
            html.Div([
                dcc.Graph(id='sales-trend',style={'width': '55%', 'display': 'inline-block','marginRight': '2%'}),
                dcc.Graph(id='profit-by-dept',style={'width': '35%', 'display': 'inline-block'})
            ...
            html.Div([
                ...
                dash_table.DataTable(
                    ...
                    style_table={  # 滚动
                        'overflowX': 'auto',  # 水平滚动
                        'overflowY': 'hidden',  # 垂直滚动
                        'maxHeight': '300px',  # 最大高
                        'width': '96%' ,
                        'margin': '0 auto'# 表格内容区域宽度（内部滚动）
                    },
                    style_header={
                    ...
                    style_data_conditional=[  # 条件样式（如数值范围）
                      ...
                      'margin': '0 auto',
                      'display': 'block'})  # 改为 block 布局（覆盖默认的 flex）
...
#
# app.layout = html.Div([
#     dcc.Graph(id='static-graph'),  # 使用静态数据的图表
#     dcc.Graph(id='dynamic-graph'),  # 使用动态数据的图表
#     dcc.Interval(id='update-trigger', interval=5*60*1000)  # 每5分钟更新一次
# ])
#
# # 静态图表：直接使用全局数据
# @callback(Output('static-graph', 'figure'), Input('dummy-trigger', 'value'))
# def update_static_graph(_):
#     return create_static_figure(static_data)
#
# # 动态图表：按需处理数据
# @callback(Output('dynamic-graph', 'figure'), Input('update-trigger', 'n_intervals'))
# def update_dynamic_graph(n):
#     dynamic_data = fetch_live_data()  # 获取实时数据
#     processed_data = process_dynamic_data(dynamic_data)  # 处理逻辑
#     return create_dynamic_figure(processed_data)