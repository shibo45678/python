...
orders = pd.read_excel('/Users/shibo/pythonProject1/消费分层-单品/RFM.xlsx', sheet_name='input',
                       usecols=['顾客编号', '大类名称', '中类名称', '商品类型', '规格型号', '销售金额', '销售日期',
                                '商品编码','销售数量', '商品单价', '是否促销'],
                       dtype={'销售日期': 'str', '销售金额': 'float'})
print(orders.head(5))
orders.describe()  # 只数值型 不包含（字符串、分类数据）
...
Index_null = null_row.index  # 删掉顾客编号空的行+金额不可以补齐(其他忽略）
...
for index, row in discount.iterrows():  # iterrows 返回2个
    if round(row['商品单价'] * row['销售数量'], 2) == row['销售金额']:
        discount.loc[index, '是否促销'] = '否'  # 注意修改值的方式
    else:
        discount.loc[index, '是否促销'] = '是'
        if pd.isna(row['销售数量']) and (row['销售金额'] < row['商品单价']):  # isnull 改成isna()
            discount.loc[index, '是否促销'] = '是'
print(discount)
# 合并到原数据
for index in discount.index:
    orders1.loc[index, '是否促销'] = discount.loc[index, '是否促销']
    print(f"{index}:{orders1.loc[index, '是否促销']}")
Orders = orders1

'''
主线流程：品类 → 动态阈值 → 相似用户数 → 
1.高中低三层 → 统计各付费层级分布变化(高中低消费平均相似用户数变化、层级用户数占比)。
'''
df_mid_cat = Orders.groupby(['销售月份', '大类名称', '中类名称', '顾客编号']).agg({'销售金额': 'sum'}).reset_index()
...
# 少于4条记录时 IQR 无意义（全局替补：子品类缺样本，父类或兄弟类有）
# Winsorize：上下 1 % 截尾
...
def calculate_IQR(df):
    ...
    for category in df['category'].unique():
        ...
        # `MaskedArray` 是 numpy 中用于处理可能包含缺失值或无效值的数据结构
        # - 实际的数据数组`.data` 属性，- 一个布尔掩码（`.mask` 属性）
        arr_clean = arr_winsorized.data  # 提取实际数据# 修复 MaskedArray 问题：转换为普通数组
...
threshold = thres_mid_cat.merge(thres_big_cat.rename(columns={'category': '大类名称', 'IQR': 'IQR_大类'}), on='大类名称', how='left')
...
'''读取人工调整后的文件，重做IQR'''
group_labels_adjust = pd.read_excel('/Users/shibo/pythonProject1/消费分层-单品/group_labels_adjust.xlsx',
                                    usecols=['category','labels','virtual'])
...
def range_optimized(df):
    ...
    for _, group in df.groupby(['销售月份', 'threshold_group'], sort=False):
        ...
        in_range = ((sales[:, None] >= sales - prec) &  # 不需要显式写成 sales[None, :] - prec
